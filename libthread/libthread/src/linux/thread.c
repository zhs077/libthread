...
void* un_thread_start(void* arg)
{
	...
	thread_cxx* cxx = (thread_cxx*)arg;
	thread_cxx cxx_p = *cxx;
	free(cxx);
	cxx_p.entry(cxx_p.arg);
...
{
	thread_cxx* cxx = (thread_cxx*)malloc(sizeof(thread_cxx));
	if (cxx == NULL)
	{
		return UN_ENOMEM;
	}
...
int un_thread_mutex_init(un_mutex_t* mutex)
{
...
void un_mutex_destory(un_mutex_t* mutex)
{
